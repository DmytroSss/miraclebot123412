                ...
                if(data) {
                    ...
                    message.channel.send(e).then(msg => {
                        ...
                        ok.on("collect", () => {
                            ...
                            .setColor(colors.default)
                            .setTitle(" Кубики ")
                            .setThumbnail('https://media.discordapp.net/attachments/837916753346035752/837921920317784094/dice.gif')
                            for (let i = 0; i < 6; i++) {
                            ...
                            if(rUserCount > mUserCount) {
                                rgameProcess += `${message.member} выиграл у ${memb.user} **${args[1]}**${bot.values.crown}`
                                res.crown += parseInt(args[1])
                                res.save()
                                data.crown -= parseInt(args[1])
                                data.save()
                            } else if(mUserCount > rUserCount) {
                                rgameProcess += `${memb.user} выиграл у ${message.member} **${args[1]}**${bot.values.crown}`
                                res.crown -= parseInt(args[1])
                                res.save()
                                data.crown += parseInt(args[1])
                                data.save()
                            ...
                            msg.edit(game.setDescription(rgameProcess));
                            no.stop()
                            return ok.stop()
                        })
                        no.on("collect", () => {
